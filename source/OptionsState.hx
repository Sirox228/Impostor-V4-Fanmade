...
class NotesSubstate extends MusicBeatSubstate
{
	...
	private var grpNumbers:FlxTypedGroup<AlphabetW>;
	private var grpNotes:FlxTypedGroup<FlxSprite>;
	private var shaderArray:Array<ColorSwap> = [];
	var curValue:Float = 0;
	var holdTime:Float = 0;
	var hsvText:AlphabetW;
	...
	public function new() {
		...
		add(grpNotes);
		grpNumbers = new FlxTypedGroup<AlphabetW>();
		add(grpNumbers);

		for (i in 0...ClientPrefs.arrowHSV.length) {
			var yPos:Float = (165 * i) + 35;
			for (j in 0...3) {
				var optionText:AlphabetW = new AlphabetW(0, yPos, Std.string(ClientPrefs.arrowHSV[i][j]));
		...
		hsvText = new AlphabetW(0, 0, "Hue    Saturation  Brightness", false, false, 0, 0.65);
		add(hsvText);
		changeSelection();
		#if mobileC
	...
	override function update(elapsed:Float) {
		...
		if (controls.BACK || (changingNote && controls.ACCEPT)) {
			changeSelection();
			if(!changingNote) {
				grpNumbers.forEachAlive(function(spr:AlphabetW) {
					spr.alpha = 0;
				});
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	private var grpOptions:FlxTypedGroup<AlphabetW>;
	private var grpInputs:Array<AttachedTextW> = [];
	...
	public function new() {
		...
		for (i in 0...optionShit.length) {
			...
			var optionText:AlphabetW = new AlphabetW(0, (10 * i), optionShit[i], (!isCentered || isDefaultKey), false);
	...
	override function update(elapsed:Float) {
		if(rebindingKey < 0) {
			...
			if (controls.BACK) {
				...
				grpOptions.forEachAlive(function(spr:AlphabetW) {
					spr.alpha = 0;
				});
				for (i in 0...grpInputs.length) {
					var spr:AttachedTextW = grpInputs[i];
					if(spr != null) {
						spr.alpha = 0;
					}
				}
	...
	private function addBindTexts(optionText:AlphabetW) {
		var text1 = new AttachedTextW(InputFormatter.getKeyName(controlArray[grpInputs.length]), 400, -55);
		text1.setPosition(optionText.x + 400, optionText.y - 55);
		text1.sprTracker = optionText;
		grpInputs.push(text1);
		add(text1);

		var text2 = new AttachedTextW(InputFormatter.getKeyName(controlArray[grpInputs.length]), 650, -55);
	...
	function reloadKeys() {
		while(grpInputs.length > 0) {
			var item:AttachedTextW = grpInputs[0];
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	private var grpTexts:FlxTypedGroup<AttachedTextW>;
	...
	public function new()
	{
		...
		grpTexts = new FlxTypedGroup<AttachedTextW>();
		add(grpTexts);

		for (i in 0...options.length)
		{
			var isCentered:Bool = unselectableCheck(i);
			var optionText:AlphabetW = new AlphabetW(0, 70 * i, options[i], false, false);
			optionText.isMenuItem = true;
			if(isCentered) {
				optionText.screenCenter(X);
				optionText.forceX = optionText.x;
			...
			if(!isCentered) {
				...
				if(useCheckbox) {
					...
					var valueText:AttachedTextW = new AttachedTextW('0', optionText.width + 80);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK) {
			...
			grpTexts.forEachAlive(function(spr:AttachedTextW) {
				spr.alpha = 0;
			});
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Change Time To Name':
						ClientPrefs.TimeToSongName = !ClientPrefs.TimeToSongName;
						
					case 'Show Distractions':
						ClientPrefs.Distractions = !ClientPrefs.Distractions;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Change Time To Name':
				daText = "If checked, the bar showing song time, will be changed on song name.";
			case 'Show Distractions':
				daText = "If checked, then the songs will contain various distracting objects.";
			case 'Optimization':
				daText = "If checked, objects will disappear in some songs, increasing performance.";
		}
		...
		for (i in 0...grpTexts.members.length) {
			var text:AttachedTextW = grpTexts.members[i];
			if(text != null) {
				text.alpha = 0.6;
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Change Time To Name':
						daValue = ClientPrefs.TimeToSongName;
					case 'Show Distractions':
						daValue = ClientPrefs.Distractions;
					case 'Optimization':
